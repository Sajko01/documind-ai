...
@Component({
  selector: 'app-dashboard',
  standalone: true,
  imports: [AsyncPipe, CommonModule, MatCardModule, DecimalPipe],
  templateUrl: './dashboard.component.html',
  styleUrl: './dashboard.component.scss'
})
export class DashboardComponent implements OnInit {
  ...
  loadPopularQuestions(): void {
    this.dashboardService.getPopularQuestions().subscribe({
      next: questions => { this.popularQuestions = questions; },
      error: error => { console.error('Popular questions failed:', error); },
    });
  }

  loadPopularDocuments(): void {
    this.dashboardService.getPopularDocuments().subscribe({
      next: documents => { this.popularDocuments = documents; },
      error: error => { console.error('Popular documents failed:', error); },
    });
  }

  loadFeedbackStatistics(): void {
    this.feedbackService.getStatistics().subscribe({
      next: (stats) => { this.feedbackStatistics = stats; },
      error: (error) => { console.error('Feedback statistics failed:', error); },
    });
  }

  loadUnansweredQuestions(): void {
    this.dashboardService.getUnansweredQuestions().subscribe({
      next: questions => { this.unansweredQuestions = questions; },
      error: error => { console.error('Unanswered questions failed:', error); },
    });
  }
}